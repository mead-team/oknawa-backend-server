...
def get_location_point(share_key, redis):
    redis_point_id_key = f"point-{share_key}"
    share_key_data = redis.get(redis_point_id_key)

    if share_key_data is None:
        raise HTTPException(status_code=404, detail="Not Found")

    response = json.loads(share_key_data.decode("utf-8"))
    return response
...
async def get_point_place(path, query):
    ...
    async with aiohttp.ClientSession() as session:
        status_code, response = await aiohttp_util.fetch(session, url, headers, query)
        ...
        for document, api_response in zip(response.get("documents"), api_responses):
            status_code, api_response = api_response
